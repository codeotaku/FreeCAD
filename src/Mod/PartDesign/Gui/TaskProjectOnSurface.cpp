    ...
    , vp(view)
{
    ...
    connect(ui->comboResult, &QComboBox::currentIndexChanged, this, [this](int index) {
        ...
        const char* mode = index == 0 ? Part::ProjectOnSurface::EdgesMode
            : index == 1             ? Part::ProjectOnSurface::FacesMode
                                     : Part::ProjectOnSurface::AllMode;
...
void TaskProjectOnSurface::updateUI()
{
    ...
    ui->comboResult->setCurrentIndex(
        strcmp(mode, Part::ProjectOnSurface::EdgesMode) == 0 ? 0
            : strcmp(mode, Part::ProjectOnSurface::FacesMode) == 0 ? 1
                                                                    : 2
    );
}
